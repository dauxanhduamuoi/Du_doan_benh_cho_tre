...
from app.services.medical_evidence_content_service import MedicalEvidenceContentService, ResolvedEvidenceContent
from app.services.pubmed_client import PubMedArticleRecord, PubMedClient, PubMedError
...
class PubMedAutoEvidenceProvider:
    ...
    def __init__(
        self,
        client: PubMedClient,
        evidence_service: MedicalEvidenceContentService,
        *,
    ...
    def discover(
        ...
        *,
        ...
        records: dict[str, tuple[PubMedArticleRecord, str, RelevanceSignals]] = {}
        ...
        for stage, query in self._query_stages(topic, aliases):
            try:
                _count, found = self.client.search(query, self.results_per_search)
            ...
            for record in found:
                ...
                records[record.pmid] = (record, stage, signals)
                ...
                resolutions += 1
                try:
                    evidence = self.evidence_service.resolve(pmid=record.pmid, abstract_text=record.abstract_text)
                ...
                usable.append(AutoEvidenceCandidate(record, evidence, trust_class, score, signals))
            ...
            for candidate in ranked:
                ...
                if any(_near_duplicate(candidate.record.title, existing.record.title) for existing in selected):
                    audits.append(AutoDiscoveryAudit(
                        ...
                        candidate.record.pmid, candidate.record.title, records[candidate.record.pmid][1],
                    ))
                    continue
                selected.append(candidate)

        selected_pmids = {item.record.pmid for item in selected}
        final_audited = {audit.pmid for audit in audits if audit.reason_code == "NEAR_DUPLICATE_SOURCE"}
        for candidate in ranked:
            if candidate.record.pmid in final_audited:
                continue
            chosen = candidate.record.pmid in selected_pmids
            audits.append(AutoDiscoveryAudit(
